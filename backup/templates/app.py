...
def chat():
    ...
    if best_match and best_score > 0.5:
        ...
        response = f"🤖: {solution}"
        if img_path:
            file_url = url_for('static', filename=f'files/{os.path.basename(img_path)}')
            response += f"\n📷: <a href='{file_url}' target='_blank'>Click here to show the File</a>"
...
def add_problem():
    if request.method == 'POST':
        ...
        if file:
            ...
            file.save(file_path)  # حفظ الملف في مجلد static/files/
            flash("File uploaded successfully!", "success")
        
        return redirect(url_for('add_problem'))  # إعادة تحميل الصفحة بعد الرفع
